...
{{#if loading}}
  <div class="min-h-screen flex flex-col items-center justify-center gap-5">
    <Spinner size="lg"/><p class="font-['Space_Mono'] text-[0.78rem] tracking-[0.15em] text-[#c9a84c]">LOADING REVIEWS...</p>
  </div>
{{:else if error}}
...
{{:else if sel}}
  <main class="pt-[72px]">
    <GuideHero {{sel}} />
    <GuideTabBar type={{sel.type}} {{qs}} active="reviews" />
    <section class="px-[70px] py-16 pb-[120px] max-w-[1400px] mx-auto">
      <div class="flex items-center gap-3 font-['Space_Mono'] text-[0.68rem] tracking-[0.2em] mb-10" style="color:{{acColor}}">
        <span class="w-9 h-px" style="background:{{acColor}}"></span>REVIEWS
      </div>
